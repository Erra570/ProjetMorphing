...
public class Triangle {
	...
	private CercleCirconscrit calculCercleCirconscrit() throws Exception {
		...
		double rayon = Math
				.abs(point_1.distance(point_2) * point_2.distance(point_3) * point_3.distance(point_1) / (2 * delta));
	...
	public Set<Point> pointNouveauTriangle(Point p, Triangle t) {
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				double denominator = ((this.point_2.getY() - this.point_3.getY())
						* (this.point_1.getX() - this.point_3.getX())
						+ (this.point_3.getX() - this.point_2.getX()) * (this.point_1.getY() - this.point_3.getY()));
				double u = ((this.point_2.getY() - this.point_3.getY())
						* (p.getX() + (1. / 10. * i) - this.point_3.getX())
						+ (this.point_3.getX() - this.point_2.getX())
								* (p.getY() + (1. / 10. * j) - this.point_3.getY()))
						/ denominator;
				double v = ((this.point_3.getY() - this.point_1.getY())
						* (p.getX() + (1. / 10. * i) - this.point_3.getX())
						+ (this.point_1.getX() - this.point_3.getX())
								* (p.getY() + (1. / 10. * j) - this.point_3.getY()))
						/ denominator;
				...
				if (u >= 0 && v >= 0 && w >= 0) {
					...
					int x = (int) Math.round(tabBarycentrique[0] * t.point_1.getX()
							+ tabBarycentrique[1] * t.point_2.getX() + tabBarycentrique[2] * t.point_3.getX());
					int y = (int) Math.round(tabBarycentrique[0] * t.point_1.getY()
							+ tabBarycentrique[1] * t.point_2.getY() + tabBarycentrique[2] * t.point_3.getY());
	...
	public static void main(String[] args) throws Exception {
		...
		for (Point m : e)
			System.out.println(
					"Le point dans le deuxième triangle a les coordonnées : (" + m.getX() + ", " + m.getY() + ")");
	}
}